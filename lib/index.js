...
const { createWatcher } = require('nukor');
...
async function lesion(rootPath, options = {}) {
  ...
  const watcher = await createWatcher(rootPath, async (changes) => {
    for (const change of changes) {
      if (change.action === 'created') {
        for (const file of await listFiles(change.newPath)) {
      ...
      if (change.action === 'deleted') {
        for (const [file, fragment] of fragments.entries()) {
          const relativePath = path.relative(change.oldPath, file);
      ...
      if (change.action === 'modified') {
        for (const [file, fragment] of fragments.entries()) {
          ...
          if (!relativePath.startsWith('..')) {
            const oldFragment = fragment;
            const newFragment = await resolveFragment(rootPath, resolvers, change.newPath);
      ...
      if (change.action === 'renamed') {
        for (const file of await listFiles(change.newPath)) {
          const oldFragmentFile = file.replace(change.newPath, change.oldPath);
  ...
  return {
    ...
    get(key) {
      return lodash.find(
        this.fragments,
        fragment => String(fragment.key) === String(lodash.toPath(key)),
      );
    },
